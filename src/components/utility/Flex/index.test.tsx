...
describe('Flex component', () => {
  ...
  it('applies aria-label and role when provided for accessibility', () => {
    const { getByTestId } = render(
      <Flex testId="flex-test" role="list" ariaLabel="Flex items">
        <div>Child 1</div>
      </Flex>
    )
    const flexElement = getByTestId('flex-test')
    expect(flexElement).toHaveAttribute('role', 'list')
    expect(flexElement).toHaveAttribute('aria-label', 'Flex items')
  })
})
