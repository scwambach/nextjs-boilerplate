...
describe('FormField component', () => {
  ...
  it('renders checkbox type correctly using a fieldset/legend group', () => {
    const choices = [
      { id: '1', copy: 'Option 1' },
      { id: '2', copy: 'Option 2' },
    ]
    ...
    const formField = getByTestId('formFieldTestId')
    expect(formField).toBeInTheDocument()
    expect(formField.tagName).toBe('FIELDSET')
    expect(formField.querySelector('legend')).not.toBeNull()
    expect(formField).toHaveClass('formField')
  ...
  it('renders radio type correctly and groups choices under one name', () => {
    const choices = [
      { id: '1', copy: 'Yes' },
      { id: '2', copy: 'No' },
    ]
    ...
    const formField = getByTestId('radioTestId')
    const inputs = formField.querySelectorAll('input[type="radio"]')
    expect(inputs.length).toBe(2)
    expect(inputs[0].getAttribute('name')).toBe(inputs[1].getAttribute('name'))
  })

  it('marks required text inputs with aria-required for assistive technology', () => {
    const { getByTestId } = render(
      <FormField
        type="text"
        label="Email"
    ...
    const input = getByTestId('emailField').querySelector('input')
