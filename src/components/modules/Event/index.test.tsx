...
describe('Event Component', () => {
  ...
  it('opens poster modal when "See Full Image" is clicked', () => {
    const { getByText, getByLabelText } = render(<Event {...eventProps} />)
    const seeFullImageButton = getByLabelText(
      'See full image of poster for Sample Event'
    )

    fireEvent.click(seeFullImageButton)
  ...
  it('gives the poster toggle button an accessible name instead of relying on the decorative overlay text', () => {
    const { getByLabelText } = render(<Event {...eventProps} />)
    const button = getByLabelText('See full image of poster for Sample Event')
    const overlaySpan = button.querySelector('span')
    expect(overlaySpan).toHaveAttribute('aria-hidden', 'true')
  })
})
