...
describe('Badge', () => {
  ...
  // Accessibility: Badge is purely presentational (a numeric indicator with
  // no interactive affordance), so no keyboard/ARIA behavior applies. We
  // assert the semantic element choice is respected so screen readers see
  // the count in the correct inline/block context the consumer intended.
  it('renders as the requested semantic element for accessible context', () => {
    const { container } = render(<Badge number={5} elementTag="span" />)
