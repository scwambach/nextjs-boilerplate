...
describe('Breadcrumbs', () => {
  ...
  it('marks the current page for assistive technology and wraps navigation in a labeled nav landmark', () => {
    const crumbs = [{ label: 'Home', href: '/' }]
    const { getByText, getByLabelText } = render(
      <Breadcrumbs crumbs={crumbs} current="Current Page" />
    )
    expect(getByLabelText('Breadcrumbs navigation').tagName).toBe('NAV')
    expect(getByText('Current Page')).toHaveAttribute('aria-current', 'page')
  })
})
