...
- Provide visible focus indicators (ensure focus styles are defined in SCSS)
...
- Support reduced motion preferences (`prefers-reduced-motion` media query in SCSS)
...
4. **Create necessary files**:
   - Always: index.tsx, styles.scss, index.test.tsx
...
2. **Find existing styles**: Search for CSS related to this component:
   - Check `src/app/globals.scss` for component-specific styles
   - Look in page-level CSS files (e.g., `src/app/(site)/[slug]/styles.scss`)
...
6. **Split the code**:
   ...
   - Extract and move component styles to styles.scss in the component directory
...
- DO NOT skip the styles.scss file even if minimal styles exist
...
- ALWAYS search for existing styles before assuming none exist (check globals.scss, page CSS, inline styles)
- ALWAYS move component-specific styles from global/page files to the component's styles.scss
...
- ALWAYS maintain the five-file structure: index.tsx, logic.tsx, styles.scss, index.test.tsx, logic.test.tsx (unless logic.tsx would be redundant)
...
4. **Styles migration summary**:
   - Where existing styles were found (e.g., "Found in `src/app/globals.scss` lines 45-78")
   - What styles were moved to styles.scss
