  ...
  return (
    <>
      ...
      <div className="container" style={{ height: '100%'}}>
        <iframe src={url} frameBorder={0} style={{width: '100%', height: '100%', overflow: 'hidden'}} />
      </div>
    </>
  )
}
...
const WebPreview = ({document}) => {
  return (<PreviewModule document={document} url={previewUrl + `?docid=${document.displayed._id}&listing=0`} />)
}

const ListingPreview = ({document}) => {
  return (<PreviewModule document={document} url={previewUrl + `?docid=${document.displayed._id}&listing=1`} />)
}
